...
- Валидации slug (уникальность, автогенерация)
- Редактирования и удаления заметок разными пользователями
"""
import pytest  # type: ignore
from http import HTTPStatus  # type: ignore

from pytest_django.asserts import (  # type: ignore
    assertRedirects,
    assertFormError
    )
...
from notes.forms import WARNING  # type: ignore
...
def test_user_can_create_note(author_client, author, form_data):
    ...
    с корректными данными и происходит редирект на страницу успеха.

    Args:
        author_client: Клиент авторизованного автора
        author: Пользователь-автор
        form_data: Данные для формы заметки
    """
...
def test_anonymous_user_cant_create_note(client, form_data):
    """
    Тест запрета создания заметки анонимным пользователем.

    Проверяет, что анонимный пользователь не может создать заметку
    и происходит редирект на страницу авторизации.

    Args:
        client: Анонимный клиент Django
        form_data: Данные для формы заметки
    """
    url = reverse('notes:add')
...
def test_not_unique_slug(author_client, note, form_data):
    ...
    Проверяет, что нельзя создать заметку с уже существующим slug
    и выводится соответствующее сообщение об ошибке.

    Args:
        author_client: Клиент авторизованного автора
        note: Существующая заметка
        form_data: Данные для формы заметки
    """
    url = reverse('notes:add')
...
def test_empty_slug(author_client, form_data):
    """
    Тест автогенерации slug при его отсутствии.

    Проверяет, что при создании заметки без slug
    он автоматически генерируется из заголовка.
...
def test_author_can_edit_note(author_client, form_data, note):
    ...
    Args:
        author_client: Клиент авторизованного автора
        form_data: Новые данные для заметки
        note: Существующая заметка для редактирования
    """
...
def test_other_user_cant_edit_note(not_author_client, form_data, note):
    """
    Тест запрета редактирования заметки другим пользователем.

    Проверяет, что пользователь не может редактировать чужие заметки
    и получает ошибку 404.

    Args:
        not_author_client: Клиент пользователя, не являющегося автором
        form_data: Данные для попытки редактирования
        note: Заметка другого пользователя
    """
    url = reverse('notes:edit', args=(note.slug,))
...
def test_author_can_delete_note(author_client, slug_for_args):
    ...
    Args:
        author_client: Клиент авторизованного автора
        slug_for_args: Slug заметки в виде кортежа для URL
    """
...
def test_other_user_cant_delete_note(not_author_client, slug_for_args):
    """
    Тест запрета удаления заметки другим пользователем.
